...
function readAndParse(dat, ext, newlayer, win, options, sidebar) {
	...
	if (ext == "xlsx") {
		...
		for (let k = 0; k < wb.SheetNames.length; k++) {
			...
			if (wb.SheetNames[k] == "O_His") {
				...
				let rows = XLSX.utils.sheet_to_csv(wb.Sheets[wb.SheetNames[k]]);
				let data = Papa.parse(rows);
				if (Number.isInteger(data.data[0][0])) {
				...
				//For ANDES output format
					...
					//Loop through and find specific columns
					for (var i = 0; i < data.data[0].length; i++) {

						//Find the Time column
						if (data.data[0][i].match("Time") != null) {

							var tcount = 0;

							while (turn[i].slice(1, turn[i].length)[tcount] != "") {
							...
							//Get the timestep range and determine the length of each set of data
							newlayer.data["history"]["t"] = turn[i].slice(1, tcount + 1);
							dlength = newlayer.data["history"]["t"].length;
						...
						//Theta column
						if ((data.data[0][i].match("a Bus") != null) && (astart == -1)) {

							console.log("a Bus");
							astart = i;

							//Determine how many buses are used for the theta column
							if (anum == -1) {

								var j = i;
								var count = 0;

								while (data.data[0][j].match("a Bus") != null) {
						...
						//Magnitude column
						if ((data.data[0][i].match("v Bus") != null) && (vstart == -1)) {
							...
							if (vnum == -1) {
								...
								while (data.data[0][j].match("v Bus") != null) {
						...
						//Frequency column
						if ((data.data[0][i].match("f Bus") != null) && (fstart == -1)) {
							...
							if (fnum == -1) {
								...
								while (data.data[0][j].match("f Bus") != null) {
					...
					//Set the busnum to whichever variable had the most entries
					if (anum > vnum) {
					...
					//If there aren't any variables to work with, don't try to animate anything
					if (busnum == -1) {
					...
					//Get all of the variable data for each timestep
					for (var j = 0; j < dlength; j++) {
						...
						if (vstart != -1) {
							...
							temp1 = data.data[j + 1].slice(vstart, vnum + vstart);

							//If the amount of columns for a given variable is less than the assumed bus amount pad it until it's the right size
							while (temp1.length < busnum) {
							...
							//If there is no data at all, just zero it out
							temp1 = new Array(busnum).fill(0);
						}

						if (astart != -1) {

							temp2 = data.data[j + 1].slice(astart, anum + astart);

							while (temp2.length < busnum) {
							...
							temp2 = new Array(busnum).fill(0);
						}


						if (fstart != -1) {

							temp3 = data.data[j + 1].slice(fstart, fnum + fstart);

							while (temp3.length < busnum) {
							...
							temp3 = new Array(busnum).fill(0);
						}


						//Put all the data into a single array and add it to history
						tempfin = temp1.concat(temp2);
						temp = tempfin.concat(temp3);
						newlayer.data["history"]["varvgs"].push(temp);
					}	
				}
			}
		...
		//CSV's are assumed to be simulation data

		//Set up newlayer for simulation
		newlayer.sim = true;
		newlayer.time = 0.0;
		newlayer.timescale = 1;
		...
		newlayer.data["history"] = {};
		let data = Papa.parse(dat.target.result);

		//For AGVis history format
		if (Number.isInteger(data.data[0][0])) {
...
//Add the UI for multilayer features based on whether there is IDR data or not
function setupLayer(newlayer, win, options, sidebar) {
...
//Increments through the uploaded files (has to be recursive due to async)
function stepRead(addlayer, newlayer, win, options, sidebar) {
	...
	var file = addlayer.shift();
	var ext = "";
	var reader = new FileReader();
	reader.onloadend = function (dat) {
	...
	//Get the extension, make sure it's valid, then read based off of it
	tempname = file.name;
	ext = tempname.substring(tempname.lastIndexOf(".") + 1);
...
function addSidebarLayers(win, options, sidebar) {
	...
	opt_addlayer_input.multiple = true;
	document.body.appendChild(opt_addlayer_input);
	...
	opt_addlayer_input.onchange = function() {
		...
		if (opt_addlayer_input.files.length > 0) {
			...
			for (let i = 0; i < opt_addlayer_input.files.length; i++) {

				arr.push(opt_addlayer_input.files[i]);
			}

			stepRead(arr, newlayer, win, options, sidebar);
		}
	};
