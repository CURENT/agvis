...
dev() {
	...
	tmux send-keys -t2 "sleep 1 && ./go.sh run_8811 websockify $((port+1)) 127.0.0.1:$((port+9))" Enter
	tmux send-keys -t3 "sleep 1 && ./go.sh run_8812 websockify $((port+2)) 127.0.0.1:$((port+9))" Enter
