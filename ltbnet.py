...
def broadcastCommFiles(rootFolder="./", port=8819):
  ...
  with open(flow, 'r') as f:
    ...
    for row in reader:
      ...
      flowTime -= 1559160380
      flowDict[aggregateTime].append((flowTime, FromType, FromIndex, ToType, ToIndex, flowPackets, flowBytes))
  ...
  while(True):
    ...
    if newTime != oldTime:
      if newTime in flowDict:
        ...
        for i, (flowTime, FromType, FromIndex, ToType, ToIndex, flowPackets, flowBytes) in enumerate(flowDict[newTime]):
